...
def classify(method, x_train, x_test, y_train, y_test, nClasses):
    error = 0
    if(method == 'LogisticRegression'):
        ...
        multiGaussClassify = MultiGaussClassify(nClasses)
        multiGaussClassify.fit(x_train, y_train, False)
        y_pred = multiGaussClassify.predict(x_test)
        misClassifiedResponses = getMisClassifiedResponses(y_test, y_pred)
        error = np.mean(misClassifiedResponses)
        
    elif(method =='multigaussdiagclassify'):
        multiGaussDiagClassify = MultiGaussClassify(nClasses)
        multiGaussDiagClassify.fit(x_train, y_train, True)
        y_pred = multiGaussDiagClassify.predict(x_test)
        misClassifiedResponses = getMisClassifiedResponses(y_test, y_pred)
        error = np.mean(misClassifiedResponses)
